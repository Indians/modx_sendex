...
class officeModExtraController extends officeDefaultController {
	...
	public function defaultAction() {
		...
		*/
		...
		if ($css = trim($this->modx->getOption('office_sendex_frontend_css', null, MODX_ASSETS_URL . 'components/office/css/main/default.css', true))) {
			$this->modx->regClientCSS(str_replace($config['pl'], $config['vl'], $css));
		}
		if ($js = trim($this->modx->getOption('office_sendex_frontend_js', null, MODX_ASSETS_URL . 'components/sendex/js/office/default.js'))) {
	...
	public function loadPackage() {
		$corePath = $this->modx->getOption('sendex.core_path', null, $this->modx->getOption('core_path') . 'components/sendex/');
